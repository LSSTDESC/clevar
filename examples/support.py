...
def gen_cluster(
    ...
    if logm_min0 < 12:
        raise ValueError(
            "Minimum mass in computation ({logm_min0}) is too low."
    ...
    Data0["MASS_ERR"] = Data0["MASS"] * np.random.normal(
        loc=lnm_scatter + 0 * Data0["MASS"], scale=0.05
    )
    Data0["Z_ERR"] = (1 + Data0["Z"]) * np.random.normal(
        loc=z_scatter + 0 * Data0["Z"], scale=0.005
    )
    ...
    input2 = Table(
        {
            "RA": np.random.normal(loc=Data0["RA"][mask2], scale=1 / 3600.0),
            "DEC": np.random.normal(loc=Data0["DEC"][mask2], scale=1 / 3600.0),
