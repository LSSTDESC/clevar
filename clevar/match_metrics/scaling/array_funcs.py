...
def _add_bindata_and_powlawfit(ax, values1, values2, err2, log=False, **kwargs):
    ...
    if add_fit:
        ...
        fit, cov = curve_fit(pw_func, vbin_1, vbin_2, sigma=vbin_err2, absolute_sigma=True)
        ...
        fit_label = (
            rf"${avg_label}=10^{{{fit1_lab}}}\;({xl})^{{{fit0_lab}}}$"
            if log
    ...
    **kwargs,
    ...
    info.update(
        _add_bindata_and_powlawfit(
            info["ax"], values1, values2, **{k[4:]: v for k, v in kwargs.items() if k[:4] == "fit_"}
        )
    )
    return info
...
def plot_panel(
    ...
    **fit_kwargs:
    ...
    return _plot_panel(
        ...
        plot_function=plot,
        values_panel=values_panel,
        bins_panel=bins_panel,
        panel_kwargs_list=panel_kwargs_list,
        panel_kwargs_errlist=panel_kwargs_errlist,
...
def _plot_metrics(
    ...
    for metric in metrics:
        ...
        if metric_name in ("mean", "std", "median", "count", "sum", "min", "max"):
            ...
            q1 = binned_statistic(
                values1, values, bins=edges, statistic=lambda x: np.quantile(x, 0.5 * (1 - p))
            )[0]
            q2 = binned_statistic(
                values1, values, bins=edges, statistic=lambda x: np.quantile(x, 0.5 * (1 + p))
            )[0]
...
def plot_density_dist(
    ...
    **fit_kwargs:
    ...
    for v in fit_bins1:
        info["axes"]["main"].axvline(v, **updated_dict({"lw": 0.5, "color": "0"}, vline_kwargs))
    # Dist plot
    ...
    for ax, mask, lkwarg in zip(info["axes"]["top"], masks1, fit_line_kwargs_list):
        ph.add_grid(ax)
        ph.plot_hist_line(
            *np.histogram(values2[mask], bins=fit_bins2),
            ax=ax,
            shape="line",
            rotate=True,
    ...
    if kwargs["fit_add_bindata"] and info.get("plots", {}).get("errorbar", False):
        ...
        for ax, m, b, t in zip(
            info["axes"]["top"],
            info["plots"]["errorbar"].lines[0]._y,
            info["plots"]["errorbar"].lines[1][0]._y,
            info["plots"]["errorbar"].lines[1][1]._y,
        ):
            xlim = ax.get_xlim()
            ax.axhline(m, color=color)
            ax.fill_between(xlim, b, t, alpha=0.3, lw=0, color=color)
            ax.set_xlim(xlim)
    return info
