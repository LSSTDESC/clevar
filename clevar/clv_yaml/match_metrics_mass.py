...
def run(config_file):
    ...
    for cat in ("catalog1", "catalog2"):
        ...
        mass_conf[cat]["fit_mass_bins"] = make_bins(
            mass_conf[cat]["fit_mass_bins"], mass_conf["log_mass"]
        )
        mass_conf[cat]["fit_mass_bins_dist"] = make_bins(
            mass_conf[cat]["fit_mass_bins_dist"], mass_conf["log_mass"]
        )
    ...
    fit_kwargs = {
        k: mass_conf[k] for k in ("add_bindata", "add_fit", "add_fit_err", "fit_statistics")
    }
    fit_kwargs_cat = {
        i: {
    ...
    if any(case in mass_conf["plot_case"] for case in ("density", "all")):
        print(f"\n# Mass density colors")
        conf = {"fig": plt.figure(figsize=mass_conf["figsize"])}
        ax = plt.axes()
    ...
    for i, j in ("12", "21"):
        ...
        if any(case in mass_conf["plot_case"] for case in ("zcolor", "all")):
            ...
            ax = plt.axes()
            scaling.mass_zcolor(
                c1, c2, **kwargs, ax=ax, color1=i == "1", **fit_kwargs, **fit_kwargs_cat["1"]
            )
