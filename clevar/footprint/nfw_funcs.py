...
def nfw2D_profile_unnorm(R, Rs):
    ...
    f = np.ones(x.size)
    xp, xm = x[x > 1], x[x < 1]
    f[x > 1] -= (2.0 / (xp**2 - 1.0) ** 0.5) * np.arctan((-(1.0 - xp) / (1.0 + xp)) ** 0.5)
    f[x < 1] -= (2.0 / (1.0 - xm**2) ** 0.5) * np.arctanh(((1.0 - xm) / (1.0 + xm)) ** 0.5)
    Sigma = np.ones(x.size) / 3.0
    Sigma[x != 1] = f[x != 1] / (x[x != 1] ** 2 - 1.0)
    return Sigma
