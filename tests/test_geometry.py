...
def _test_convert_units(Cosmology):
    ...
    assert_allclose(convert_units(1.0, 'degrees', 'arcmin'), 60., **TOLERANCE)
    assert_allclose(convert_units(1.0, 'degrees', 'arcsec'), 3600., **TOLERANCE)

    # Test cases to make sure physical -> physical is fitting together
    assert_allclose(convert_units(1.0, 'Mpc', 'kpc'), 1.0e3, **TOLERANCE)
    assert_allclose(convert_units(1000., 'kpc', 'Mpc'), 1.0, **TOLERANCE)
    ...
    assert_allclose(convert_units(r_arcmin, 'arcmin', 'kpc', redshift, cosmo),
                    truth, **TOLERANCE)
...
#    d_a = cosmo.angular_diameter_distance(redshift).to('kpc').value
    ...
    assert_allclose(convert_units(r_kpc, 'kpc', 'arcmin', redshift, cosmo),
                    truth, **TOLERANCE)
