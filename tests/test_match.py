...
def test_proximity(CosmoClass):
    ...
    # Check prep cat
    assert_allclose(cat2.mt_input['ang'], np.ones(cat2.size)/3600.)
    # Check multiple match
    mmt = [
        ['CL0', 'CL1', 'CL2'],
        ['CL0', 'CL1', 'CL2'],
    ...
    assert_raises(ValueError, mt.prep_cat_for_match, cat1, delta_z='cat',
    ...
    assert_allclose(cat1.mt_input['zmin'], cat1['z']-.22)
    assert_allclose(cat1.mt_input['zmax'], cat1['z']+.33)
...
def test_membership():
    ...
    mt.multiple(cat1, cat2)
    mt.multiple(cat2, cat1)
    mt.unique(cat1, cat2, 'shared_member_fraction', minimum_share_fraction=.7)
    mt.unique(cat2, cat1, 'shared_member_fraction', minimum_share_fraction=.7)
    cat1.cross_match()
    cat2.cross_match()
    print('########')
    print(cat1)
    ...
    mt.multiple(cat1, cat2)
    mt.multiple(cat2, cat1)
    mt.unique(cat1, cat2, 'shared_member_fraction', minimum_share_fraction=.9)
    mt.unique(cat2, cat1, 'shared_member_fraction', minimum_share_fraction=.9)
    cat1.cross_match()
    cat2.cross_match()
    print('########')
    print(cat1)
...
def test_membership_cfg(CosmoClass):
    ...
    cat1._init_match_vals(overwrite=True)
    cat2._init_match_vals(overwrite=True)
    match_config_test['minimum_share_fraction1_unique'] = .7
    match_config_test['minimum_share_fraction2_unique'] = .7
    ...
    cat1._init_match_vals(overwrite=True)
    cat2._init_match_vals(overwrite=True)
    match_config_test['minimum_share_fraction1_unique'] = .9
    match_config_test['minimum_share_fraction2_unique'] = .9
    mt.match_from_config(cat1, cat2, match_config_test)
    smt = [None, 'CL1', 'CL2', 'CL3', None]
    omt = ['CL0', 'CL1', 'CL2', None, None]
    cmt = [None, 'CL1', 'CL2', None, None]
...
def test_output_matched_catalog():
    ...
    assert_raises(ValueError, output_matched_catalog, file_in1, file_in2,
        file_out, cat1[:-1], cat2, 'cross')
    assert_raises(ValueError, output_matched_catalog, file_in1, file_in2,
        file_out, cat1, cat2[:-1], 'cross')
    # normal functioning
