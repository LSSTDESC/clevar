...
def test_color_panel():
    cat1, cat2 = _test_data.cat1, _test_data.cat2
    info = scaling.ClCatalogFuncs.plot_panel(cat1, cat2, 'cat1', 'mass', col_color='z', col_panel='z',
                                             bins_panel=3)
    info = scaling.ClCatalogFuncs.plot_panel(cat1, cat2, 'cat1', 'mass', col_color='z', col_panel='z',
                                             bins_panel=[3, 4, 5])
def test_m_metrics():
...
def test_m_density_metrics():
    cat1, cat2 = _test_data.cat1, _test_data.cat2
    info = scaling.mass_density_metrics(cat1, cat2, 'cat1', mask1=cat1['mass']>0, mask2=cat2['mass']>0)
def test_m_dist():
    cat1, cat2 = _test_data.cat1, _test_data.cat2
...
def test_m_dist_self():
    ...
    assert_raises(ValueError, scaling.ClCatalogFuncs.plot_dist_self,
                  cat1, col='mass', transpose=True, col_aux=None)
def test_m_density_dist():
    ...
    assert_raises(ValueError, scaling.mass_density_dist, cat1, cat2, 'cat1', add_fit=True,
                  fit_statistics='unknown')
    info = scaling.mass_density_dist(cat1, cat2, 'cat1', add_fit=True, fit_bins1=2, fit_bins2=3,
                                     fit_statistics='mode')
    info = scaling.mass_density_dist(cat1, cat2, 'cat1', add_fit=True, fit_bins1=[1e16,1e18], fit_bins2=3)
